...
int query_up(int u, int v)
{
    ...
    while(true)
    {
        ...
        {
            if(u == v) break;
            ans = max(ans, query_tree(0, now, 1, pos_in_arr[v]+1, pos_in_arr[u])); /// node er khetre +1 ta bud jabe ekhane V er edge er hishab V+1 e ache
            break;
        }
        u = chain_head[uchain];
